function [empaticaFeatureTable, caneFeatureTable] = calculateFeatures_DCN(empaticaDataExperiment, caneDataExperiment, participant_ID, winTime, winOverlapTime)
...
%% Validaciones
    ...
    for i=1:numel(experimentNames)
        ...
        canePPG_smooth=smooth(canePPG,1/3*Fs_PPG);
        empaticaPPG_smooth=smooth(empaticaPPG,1/3*Fs_PPG);
        ...
        for j=1:winLengthPPGOverlap:ppg_length - winLengthPPG + 1
            ...
            % Signals inside the window
            ppgCaneWin = canePPG(j:j+winLengthPPG-1);
            ppgEmpaticaWin = empaticaPPG(j:j+winLengthPPG-1);
            % Intersección de los tiempos de los maximos y los tiempos en la ventana
            IBICaneWin = CaneIBI(ismember(timestampCane(caneidx_max),timestampCane(j:j+winLengthPPG-1)));
            IBIEmpaticaWin = EmpaticaIBI(ismember(timestampEmpatica(empaticaidx_max),timestampEmpatica(j:j+winLengthPPG-1)));
            IBInormCaneWin = CaneIBInorm(ismember(timestampCane(caneidx_max),timestampCane(j:j+winLengthPPG-1)));
            IBInormEmpaticaWin = EmpaticaIBInorm(ismember(timestampEmpatica(empaticaidx_max),timestampEmpatica(j:j+winLengthPPG-1)));
            ...
            % Time Caracteristics
            % Feature 10: Mean IBI
            MeanIBI(countWin,1) = mean(IBInormCaneWin);
            MeanIBI(countWin,2) = mean(IBInormEmpaticaWin);
            % Feature 11: Heart Rate
            % (sin normalizar para obtener valores reales)
            HR(countWin,1) = 60/mean(IBICaneWin);
            HR(countWin,2) = 60/mean(IBIEmpaticaWin);
            % Feature 12: Standar Deviation of IBI
            SDNN(countWin,1) = sqrt(sum((IBInormCaneWin-MeanIBI(countWin,1)))^2/(winLengthPPG-1));
            SDNN(countWin,2) = sqrt(sum((IBInormEmpaticaWin-MeanIBI(countWin,2)))^2/(winLengthPPG-1));
            % Feature 13: Root Mean Square of Sucesives IBI interval diference
            RMSSD(countWin,1) = sqrt(sum((IBInormCaneWin(2:end)-IBInormCaneWin(1:end-1)).^2)/(winLengthPPG-1));
            RMSSD(countWin,2) = sqrt(sum((IBInormEmpaticaWin(2:end)-IBInormEmpaticaWin(1:end-1)).^2)/(winLengthPPG-1));
            % Feature 14: Sucesive Differences Standardar Deviation
            SucDif_Cane=abs(IBInormCaneWin(2:end)-IBInormCaneWin(1:end-1));
            SDSD(countWin,1) = sqrt(sum((SucDif_Cane-mean(SucDif_Cane)).^2)/(winLengthPPG-2));
            SucDif_Empatica=abs(IBInormEmpaticaWin(2:end)-IBInormEmpaticaWin(1:end-1));
            SDSD(countWin,2) = sqrt(sum((SucDif_Empatica-mean(SucDif_Empatica)).^2)/(winLengthPPG-2));
            % Feature 15: Percent of IBI larger than 20ms
            % (sin normalizar para obtener valores reales)
            pNN20(countWin,1) = sum(IBICaneWin>0.020)/winLengthPPG*100;
            pNN20(countWin,2) = sum(IBIEmpaticaWin>0.020)/winLengthPPG*100;
            % Feature 16: Percent of IBI larger than 50ms
            % (sin normalizar para obtener valores reales)
            pNN50(countWin,1) = sum(IBICaneWin>0.050)/winLengthPPG*100;
            pNN50(countWin,2) = sum(IBIEmpaticaWin>0.050)/winLengthPPG*100;
            % Feature 17: Baevsky's Stress Index
            % Calculamos el hsitograma primero: incremeto de 50ms
            t_intervalos=0.05;
            if(length(IBInormCaneWin)>2)
                caneintervalos=(min(IBInormCaneWin):t_intervalos:max(IBInormCaneWin))+t_intervalos/2;% Puntos centrales de los rangos
                if(length(caneintervalos)<2)
                    % Si es muy pequeño, lo dividimos en dos regiones
                    incremento_t =(max(IBInormCaneWin)-min(IBInormCaneWin))/4;
                    caneintervalos = [min(IBInormCaneWin)+incremento_t max(IBInormCaneWin)-incremento_t]; % Puntos centralos a los rangos
                end
                canehist = histcounts(IBInormCaneWin, caneintervalos);
                [caneAMo_pre, canehist_idx] = max(canehist);
                caneMo = caneintervalos(canehist_idx);
                caneAMo = caneAMo_pre/winLengthPPG;
                caneMxDMn = max(IBInormCaneWin) - min(IBInormCaneWin);
                SI(countWin,1) = (caneAMo/(2*caneMo*caneMxDMn))*100; % Para el bastón 
            else
                SI(countWin,1) = 0;
            end
            if(length(IBInormEmpaticaWin)>2)
                empaticaintervalos=(min(IBInormEmpaticaWin):t_intervalos:max(IBInormEmpaticaWin))+t_intervalos/2;% Puntos centrales de los rangos
                if(length(empaticaintervalos)<2)
                    % Si es muy pequeño, lo dividimos en dos regiones
                    incremento_t =(max(IBInormEmpaticaWin)-min(IBInormEmpaticaWin))/4;
                    empaticaintervalos = [min(IBInormEmpaticaWin)+incremento_t max(IBInormEmpaticaWin)-incremento_t]; % Puntos centralos a los rangos
                end
                empaticahist = histcounts(IBInormEmpaticaWin, empaticaintervalos);
                ...
                empaticaMxDMn = max(IBInormEmpaticaWin) - min(IBInormEmpaticaWin);
